...
from api.arenas import schemas as arenas_schemas
...
class ModelSchemaHelper:
    ...
    def __init__(self, name: str, short_name: str, metric: str | int, request: Request):
        self.name = name
    ...
    def filter_params_for_where_orm_objects(
        ...
        # ands = [self.orm_model.idarena == id_filter, ]
        ands = []
        for param_name, param_value in self.request.path_params.items():
    ...
    def populate_with_params(self,
        ...
        try:
            ...
            self.check_for_invalid_query_params()
...
async def get_helper_obj(
        ...
        request: Request):
    yield ModelSchemaHelper('arenas', 'arena', 'games', request)


async def get_helper_nometric_obj(
        # metric: Annotated[Literal[*metrics], Path()],
        request: Request):
    yield ModelSchemaHelper('arenas', 'arena', 'tnt', request)
